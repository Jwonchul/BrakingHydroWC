...
# # Performance Index
# df_total = []
# srtt_df = df[df['GroupSpec'] == 'SRTT'].copy()
#
# for name, grp in df.groupby(['TestItem','ReqNo','RoadInfo']):
#     # std_row = grp[grp['GroupSpec'] == 'SRTT'].iloc[0]
#     if 'SRTT' in grp['GroupSpec'].values:
#         std_row = grp[grp['GroupSpec'] == 'SRTT'].iloc[0]
#     else:
#         time_mean = grp['Datetime'].mean()
#
#         # 전체 SRTT 데이터에서 가장 가까운 Datetime 찾기
#         closest_idx = (srtt_df['Datetime'] - time_mean).abs().idxmin()
#         std_row = srtt_df.loc[closest_idx]
#
...
# Performance Index (SRTT 없는 경우)
df_first = df

# Ax 변화가 심한 경우 제외
# df_first['CompareAx'] = abs(1-df_first['meanAx']/df_first['rmsAx'])*100
# # df_final = df_first[abs(df_first['CompareAx'])<2.5] # 조건 대입
# df_final_total = df_first # 모든 조건
if 'meanAx' in df_first.columns and 'rmsAx' in df_first.columns:
    # Ax 변화가 심한 경우 제외
    df_first['CompareAx'] = abs(1 - df_first['meanAx'] / df_first['rmsAx']) * 100
    # df_final = df_first[abs(df_first['CompareAx']) < 2.5]
    df_final_total = df_first  # 모든 조건
else:
    # meanAx 칼럼이 없으면 그냥 원본 그대로 사용
    df_final_total = df_first.copy()

# 변동계수 구하기
df_CV = df_final_total.copy()
df_CV['CV'] = (df_CV['Dist_std']/df_CV['Dist_mean'])*100
df_CV['CV_mean'] = df_CV['CV'].mean()*2

## 임시
plt_group(df_first, ['GroupSpec'], 'RoadInfo', 'DHC', 'Dist_mean')
plt_group_multi(df_first,['RoadInfo', 'GroupSpec'],'DHC','Dist_mean',text='AMPM')
plt_group_multi(df_first,['RoadInfo', 'GroupSpec'],'DHC','TempSEP',text='AMPM')
# plt_group(df_first, ['GroupSpec'], 'RoadInfo', 'DHC', 'Dist_mean')
