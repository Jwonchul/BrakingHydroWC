...
select_plt_col = ['VelHorizontal', 'filterAcc', 'dist']
...
# 파일명에 따라 폴더 만들고 저장
    ...
    df_time_sum['dist'] = df_time_sum['dist'].cumsum()
    ...
    gps_col = next((c for c in dfbrk.columns if c in ['GPS_Sats', 'gps-sats']), None)
    if gps_col:
        df_dist['GPS_Sum'] = dfbrk[gps_col].mean()
        df_dist['GPS_Max'] = dfbrk[gps_col].max()
        df_dist['GPS_Min'] = dfbrk[gps_col].min()
    else:
        df_dist['GPS_Sum'] = np.nan
        df_dist['GPS_Max'] = np.nan
        df_dist['GPS_Min'] = np.nan
    ...
    if any('WheelSpeed' in col for col in rawdf[k][1].columns) == True :
        # Partial Hydro
        ...
        df_slip_result['dfbrk'] = [dfbrk]
        df_slip_result['df_time_sum'] = [df_time_sum]
        # brkraw = df_slip_result['dfbrk'].iloc[0]
        # frontslipRaw = df_slip_result['Front_df_AN'].iloc[0]
...
# plt.figure()
# plt.plot(slip_results[0]['Front_df_AN'][0][:0],slip_results[0]['Front_df_AN'][0].iloc[1],'.')
...
save_pickle_total = os.path.join(os.path.dirname(fName[0]), "Brk_data_total.pkl")
with open(save_pickle_total, "wb") as f:
    pickle.dump(df_slip_total, f)

df_all['Dist_range']=df_all['Dist_max']-df_all['Dist_min']
df_all_select = df_all[df_all['RoadInfo']=='B1C220']
df_all_K135 = df_all[df_all['Spec1']=='K135']
df_all_select2 = df_all_select[['CondInfo','Spec1','Spec2','Dist_mean','perDist']]
df_all['Dist_range']=df_all['Dist_max']-df_all['Dist_min']
## ANOVA
