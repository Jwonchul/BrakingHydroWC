...
folder_path = os.path.join(os.path.dirname(fName[0]))
...
plt_stats_bar_v2(df_t,'GroupSpec','AMPM','Dist_mean',st='RoadInfo')

plt_group_multi(df_final,['RoadInfo', 'GroupSpec'],'perDist','Dist_mean',text='RoadInfo')
plt_group_multi(df_final,['GroupSpec'],'perDist','Dist_mean',text='RoadInfo')
...
def fit_linear_models(df,x_range,x_col,y_cols,group_name,**kwargs):
    ...
    for y_col in y_cols:
        ...
        X_train = valid_df[[x_col]]
        y_train = valid_df[y_col]
        ...
        # 한 행으로 요약 dict에 추가
        dict_summary[f"{y_col}_R2"] = r2
        dict_summary[f"{y_col}_Slope"] = slope
        dict_summary[f"{y_col}_10"] = x_per_10_y
    ...
    if plot:
        ...
        for i, ch in enumerate(df_result.columns[1:]):
            clean_ch = ch.replace('_fit', '')
            plt.plot(raw_df[f"{x_col}"], raw_df[clean_ch],'o', color=color_list[i], label=clean_ch)
            chname = str(round(df_summary[f"{clean_ch}_10"].iloc[0], 2))+'/10°C'
            plt.plot(df_result[f"{x_col}_fit"], df_result[ch],'-', color=color_list[i], label=chname)
        ...
        min = round(raw_df[f"{x_col}"].min()-2, 0)
        max = round(raw_df[f"{x_col}"].max()+2, 0)
        plt.xlim([min,max])
